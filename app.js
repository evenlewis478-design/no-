...
var MODE_DESC = {
  'Normal':'Full visual quality. Matches Roblox defaults. Best for strong PCs.',
  'Low':'Light cuts on the heaviest effects. Small FPS gain, almost no visual loss.',
  'Potato':'Balanced cuts across the board. The recommended starting point for most low-end PCs.',
  'Extreme Potato':'Heavy cuts everywhere. Roblox will look plain, but should run much smoother.',
  'Absolute Potato':'Everything reduced as far as it goes. Maximum performance, minimum visuals.'
};

var STAGES = ['Reading current profile','Calculating target values','Reducing heavy effects','Lowering shadows & lighting','Simplifying particles & post-FX','Building recommended checklist','Complete'];
...
function statusForAvg(avg){
  if (avg >= 80) return {label:'High Quality', color:'var(--text)'};
  if (avg >= 50) return {label:'Balanced', color:'var(--accent)'};
  if (avg >= 25) return {label:'Performance', color:'var(--accent-2)'};
  return {label:'Max Performance', color:'var(--accent-2)'};
}
...
function renderDashboard(){
  ...
  var hero = el('div', {class:'hero'}, [
    el('div', {}, [
      ...
      el('p', {}, [txt('Plan lower graphics settings for Roblox so the game runs smoother on your PC. Profiles, modes, and clear estimates — you still apply the changes inside Roblox yourself.')])
    ]),
  ...
  statGrid.appendChild(statCard('Current Focus', status.label, 'Based on current settings', status.color));
...
function renderOptimizerPage(){
  ...
  note.appendChild(el('p', {style:'font-size:13px;color:var(--text-dim);margin:0;'}, [
    txt('This tool cannot open Roblox or change settings for you — no website can do that safely. It calculates recommended values for your chosen mode, saves them to the active profile, and shows a short checklist of what to set manually inside Roblox (Settings → Graphics).')
  ]));
...
function runOptimizer(mode){
  ...
  function finish(){
    ...
    var robloxLevel = Math.max(1, Math.min(10, Math.round(afterPerf.avg / 10)));
    resultEl.style.display = '';
    resultEl.innerHTML =
      '<div style="margin-top:14px;padding-top:14px;border-top:1px solid var(--line);">' +
      '<b style="font-family:Sora;font-size:15px;color:var(--accent-2);">Optimization complete</b>' +
      '<p style="font-size:13px;color:var(--text-dim);margin:8px 0 0;">Mode applied: <b style="color:var(--text);">' + state.activeMode + '</b> · Profile: ' + getActiveProfile().name + '</p>' +
      '<p style="font-size:13px;color:var(--text-dim);margin:4px 0 0;">Estimated FPS: ' + beforePerf.fps + ' → ' + afterPerf.fps + ' (' + (delta >= 0 ? '+' : '') + delta + ')</p>' +
      '<p style="font-size:13px;color:var(--text-dim);margin:4px 0 0;">Optimization score: ' + afterPerf.score + ' / 100</p>' +
      '<div style="margin-top:12px;padding:12px;border-radius:8px;background:var(--panel-2);border:1px solid var(--line);">' +
      '<div style="font-size:12.5px;font-weight:600;margin-bottom:6px;color:var(--text);">What to do in Roblox</div>' +
      '<p style="font-size:12.5px;color:var(--text-dim);margin:0 0 6px;">1. Open Roblox → Settings (Esc) → Graphics</p>' +
      '<p style="font-size:12.5px;color:var(--text-dim);margin:0 0 6px;">2. Set Graphics Mode to <b style="color:var(--text);">Manual</b></p>' +
      '<p style="font-size:12.5px;color:var(--text-dim);margin:0 0 6px;">3. Move the quality slider to around <b style="color:var(--text);">level ' + robloxLevel + '</b> (1–10)</p>' +
      '<p style="font-size:12.5px;color:var(--text-dim);margin:0;">4. Turn off extra effects if still lagging (shadows, particles, etc.)</p>' +
      '</div>' +
      '<p style="font-size:11.5px;color:var(--text-faint);margin:10px 0 0;">These numbers are estimates only. This tool cannot change Roblox settings for you.</p>' +
      '</div>';
...
function openAboutModal(){
  ...
  body.appendChild(el('p', {}, [txt('Roblox Potato Graphics is a free settings planner. It cannot open the Roblox client, inject FastFlags, edit files, or change settings for you — no website can do that safely. Any tool that claims otherwise is not safe to use.')]));
  body.appendChild(el('p', {}, [txt('What it does do: recommends which in-game graphics options to lower based on the mode you pick, lets you keep separate profiles per game, and shows estimated FPS/score so you can decide if the trade-off is worth it.')]));
  body.appendChild(el('p', {}, [txt('All FPS, GPU, CPU, and memory numbers are calculated estimates only — never a live reading of your hardware. After optimizing here, open Roblox → Settings → Graphics and set the quality slider (and any extra effects) yourself.')]));
